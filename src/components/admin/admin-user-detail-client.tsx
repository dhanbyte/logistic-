...
import type { AdminUserDetailData } from "@/lib/data/admin/users";

export function AdminUserDetailClient({ initialData }: { initialData: AdminUserDetailData }) {
  const [fullName, setFullName] = useState(initialData.fullName);
  const [email, setEmail] = useState(initialData.email);
  const [phone, setPhone] = useState(initialData.phone);
  const [companyName, setCompanyName] = useState(initialData.companyName);
  const [status, setStatus] = useState(initialData.status);
  const [billingMode, setBillingMode] = useState(initialData.billingMode);
  ...
  async function handleToggleStatus() {
    ...
    setLoading(true);
    const res = await toggleUserStatusAction(stats.userId, fullName, newStatus);
    setLoading(false);
    if (res.ok) {
      setStatus(newStatus);
      toast.success(res.message);
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            User ID: <span className="font-mono text-slate-700 font-semibold">{stats.userId}</span> &bull; {fullName} ({email})
          </p>
        </div>

        <div className="flex items-center gap-2">
          <Link
            href={`/admin/users/${stats.userId}/rates`}
          ...
          <Link
            href={`/admin/finance/wallet?userId=${stats.userId}`}
