...
import { getEcommerceShipmentById, getEcommerceShipments } from "@/lib/data/ecommerce-shipments";
...
export default async function BulkLabelsPage({
  ...
  if (idList.length > 0) {
    const fetched = await Promise.all(idList.map((id) => getEcommerceShipmentById(id)));
    shipments = fetched.map((f) => f.shipment).filter(Boolean);
  } else {
    // If no specific IDs passed, load recent manifested shipments
    const res = await getEcommerceShipments({ pageSize: 50 });
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-4 sm:p-8 flex flex-col items-center justify-start print:p-0 print:bg-white">
      ...
      <div className="mb-6 flex w-full max-w-[420px] items-center justify-between print:hidden">
        <Link
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold text-slate-700 bg-slate-200 px-2 py-1 rounded">
            {shipments.length} Labels
          </span>
          <PrintLabelButton />
        </div>
      </div>

      {/* Multiple 4x6 Inch Thermal Labels */}
      <div className="flex flex-col gap-8 print:gap-0 print:block">
        {shipments.map((shipment, index) => {
          ...
          return (
            <div
              key={shipment.id}
              className="w-full max-w-[400px] rounded-lg border-2 border-slate-900 bg-white p-4 font-sans text-slate-900 shadow-lg print:max-w-none print:w-[4in] print:min-h-[6in] print:border-none print:shadow-none print:p-2 print:break-after-page print:page-break-after-always"
            ...
            >
              ...
              <div className="flex items-center justify-between border-b-2 border-slate-900 pb-2">
                <div>
                  <h1 className="text-lg font-black tracking-tight uppercase">
                    {shipment.courierProvider?.name || (shipment.awbNumber.startsWith("SF") ? "Shadowfax Express" : "Xpressbees")}
                  </h1>
                ...
                <div className="text-right">
                  <span className="rounded border-2 border-slate-900 px-2 py-0.5 text-xs font-black uppercase">
                    {shipment.routingCode || `${shipment.deliveryPincode.slice(0, 3)}`}
                  </span>
                </div>
              </div>
              ...
              <div className="my-3 text-center border-b-2 border-slate-900 pb-3">
                ...
                <p className="text-[10px] font-semibold text-slate-500">
                  Order Ref: {order?.orderNumber || `ORD-${index + 1}`}
                </p>
              </div>
              ...
              <div className="pt-2 text-[9px] text-slate-600">
                ...
                <p className="font-bold text-slate-900">
                  {warehouse?.warehouseName || "Dhanbyte Logistics Warehouse"}
                </p>
        ...
        {!shipments.length && (
          <div className="rounded-xl border border-slate-200 bg-white p-12 text-center text-slate-500">
            <p className="text-sm font-semibold">No manifested shipments found.</p>
            <p className="text-xs text-slate-400 mt-1">Book orders first to generate shipping labels.</p>
          </div>
        )}
      </div>
    </div>
