import { Banknote, CheckCircle2, Clock, Download, IndianRupee, Package, ShieldCheck } from "lucide-react";
...
export default async function CodPage() {
  ...
  const settlements = userId ? await getMerchantCodSettlements(userId) : [];

  const pendingCodPaise = settlements
    .filter((s) => s.status !== "PAID")
    .reduce((acc, s) => acc + s.codAmountPaise, 0);
    ...
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-3 mb-6">
        <div className="rounded-xl border border-teal-200 bg-teal-50/60 p-4 shadow-xs">
          ...
          <p className="text-2xl font-black text-slate-900 mt-1">{formatINR(pendingCodPaise / 100)}</p>
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-xs overflow-hidden">
        ...
        {settlements.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <h4 className="text-sm font-semibold text-slate-700">No COD Shipments Yet</h4>
            <p className="text-xs text-slate-400 max-w-sm mx-auto mt-1">
              When you ship Cash on Delivery (COD) orders, their delivery status, cash collection, freight deductions, and bank payout UTRs will appear here.
            </p>
          </div>
        ) : (
