...
import { Banknote, Eye, Package, Search } from "lucide-react";
import { formatINR } from "@/lib/calculations";
import { getOrders } from "@/lib/data/orders";

export default async function AdminCodOrdersPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Cash on Delivery (COD) Orders</h1>
          <p className="text-xs text-slate-500">
            Monitor cash collections, pending courier remittances, and delivery confirmations.
          </p>
        </div>
      </div>
        ...
        <table className="w-full text-left text-xs">
          <thead className="border-b border-slate-200 bg-slate-50 font-semibold text-slate-700">
            <tr>
              ...
              <th className="py-3 px-4">COD Collectible</th>
          ...
          <tbody className="divide-y divide-slate-100 text-slate-600">
            {codOrders.map((o) => (
              <tr key={o.id} className="hover:bg-slate-50/70">
                ...
                <td className="py-3 px-4">{o.customer?.fullName} &bull; PIN: {o.customer?.pincode}</td>
                <td className="py-3 px-4 font-black text-amber-700">{formatINR(o.codAmount || o.orderAmount)}</td>
